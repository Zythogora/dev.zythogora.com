  ...
  await prisma.$transaction(
    async (tx) => {
      await tx.users.upsert({
        where: { id: "database_seeding" },
        update: {},
        create: {
          id: "database_seeding",
          username: "database_seeding",
        },
      });
    ...
    `;
      ...
      await tx.$executeRaw`
      INSERT INTO "beer_data"."breweries" ("id", "previous_id", "name", "country_alpha_2_code", "created_by", "updated_by")
      VALUES ${Prisma.join(
        existing_breweries.map(
          (brewery) =>
            Prisma.sql`(${nanoid()}, ${Number(brewery.id)}, ${brewery.name}, ${brewery.country_alpha_2_code}, 'database_seeding', 'database_seeding')`,
        ),
      )}
      ON CONFLICT (name) DO UPDATE SET previous_id = EXCLUDED.previous_id;
    `;
      ...
      await tx.beers.createMany({
        data: beersToInsert.map((beer) => ({
          ...
          createdBy: "database_seeding",
          updatedBy: "database_seeding",
        })),
      });
